...
class ConfirmActionManager {
    ...
    private function getPurchaseData($form){
        ...
        // 商品情報クラスを生成
        $commodityDetails = $this->getCommodityDetail($form);
        ...
        // 利用可能な決済種別
        $settlementTypeList = array(
                AW_SETTLEMENT_TYPE_CARD,
                AW_SETTLEMENT_TYPE_CVS 
        );
        ...
        // ダミー取引フラグ
        $data->setDummyPaymentFlag(DUMMY_PAYMENT_FLAG);
     ...
     * @return 購入商品情報
     */
    private function getCommodityDetail(PurchaseForm $purchaseForm){
        $log = $this->log->getMethodLog(__METHOD__);
        $log->info("購入商品情報クラスをセット");
        
        $commodityDetails = array();
        // 商品数
        $listsize = $purchaseForm->getNamelistSize();
        for($idx = 0; $idx < $listsize; $idx++) {
            $setId = null;
            if ($purchaseForm->getIdmap($idx + 1) == null || $purchaseForm->getIdlistSize() == 0) {
                $setId = DUMMY_COMMODITY_ID . $idx + 1;
            } else {
                $setId = $purchaseForm->getIdmap($idx + 1);
            }
            
            $setJanCode = null;
            if ($purchaseForm->getJanCodemap($idx + 1) == null || $purchaseForm->getJanCodelistSize() == 0) {
                $setJanCode = DUMMY_COMMODITY_JANCODE . $idx + 1;
            } else {
                $setJanCode = $purchaseForm->getJanCodemap($idx + 1);
            }
            
            // 商品情報を登録
            $commodityDetails[] = new CommodityDetail($this->getSorpCode($setId), $this->getSorpCode($setJanCode), $this->getSorpCode($purchaseForm->getNamemap($idx + 1)), $this->getSorpCode($purchaseForm->getUnitmap($idx + 1)), $this->getSorpCode($purchaseForm->getNummap($idx + 1)));
        }
    ...
    private function sendPOST(PurchaseData $data){
        ...
        // ダミー取引フラグ
        $query['DUMMY_PAYMENT_FLAG'] = $data->getDummyPaymentFlag();
        // 商品情報を登録する
        ...
        if (intval($code) == 200) {
            ...
            if (!is_null($merchantKey) && !is_null($browserKey)) {
                $this->merchant->setMerchantEncKey($merchantKey);
                $this->merchant->setBrowserEncKey($browserKey);
    ...
    public function setMerchantKey(){
        ...
        $dataStore = $dataStoreHelper->getKey($this->merchant->getOrderID());
        $dataStore->setMerchantEncKey($this->merchant->getMerchantEncKey());
        $dataStore->setSessionId($this->merchant->getSessionId());
        $dataStoreHelper->setKey($dataStore);
        $log->info("DataStoreデータ登録 取引ID：" . $dataStore->getOrderId());
        $log->info("DataStoreデータ登録 マーチャント暗号鍵：" . $dataStore->getMerchantEncKey());
        $log->info("DataStoreデータ登録 セッションID：" . $dataStore->getSessionId());
    }
}
